...
double compute_llhood(std::vector<double>& xx, double b, std::vector<double> & yy, std::vector<sparse_array> & rows) {
  ...
  for(int i=0; i<rs; i++) {
    ...
    llhood -= logloss(-yy[i]*(Ax+b));
  }
  return llhood;
}

double compute_objective_logreg(double lambda, double * l1x = NULL, double * loglikelihood = NULL, int * _l0 = NULL, double * testobj = NULL) {
  ...
  double llhood = compute_llhood(logregprob->x, logregprob->b, logregprob->y, logregprob->A_rows);
